...
#determines which adjacent triangles of a given triangle are already in a strip
#eliminates non valid triangles from the possible adjacent triangles

def validAdjTri(triangle):
    valid = []
    for k in triangle.adjTris:
        if k.nextTri == None and k.prevTri == None:
            valid.append(k)
    return valid



#builds an individual tristrip using recursion

def buildTristrip(triangles, current_triangle):
    all_adj = validAdjTri(current_triangle)


    #if only one valid triangle, no need for more optimization

    if len(all_adj) == 1:
        adj = all_adj[0]

        current_triangle.nextTri = adj
        adj.prevTri = current_triangle
        adj.nextTri = buildTristrip(triangles, adj)


    #if there are 2 next triangle options, pick the one with the 
    #least amount of valid next triangles

    elif len(all_adj) == 2:   
        if len(validAdjTri(all_adj[0])) > len(validAdjTri(all_adj[1])):
            adj = all_adj[1]
        else:
            adj = all_adj[0]
    ...
    return current_triangle.nextTri

def buildTristrips(triangles):
    count = 0

    #puts the triangles into 1 of 3 lists based on it's number of adjacent triangles

    one_adj = []
    two_adj = []
    three_adj = []

    for start_point in triangles:
        if len(start_point.adjTris) == 1:
            one_adj.append(start_point)
        elif len(start_point.adjTris) == 2:
            two_adj.append(start_point)
        elif len(start_point.adjTris) == 3:
            three_adj.append(start_point)

    #combines the 3 lists into 1 list for ease of use
    #start of list is 1 adjacent triangle, followed by 2 then 3

    all_adj = one_adj + two_adj + three_adj

    #iterates through the sorted list looking for available triangles
    #to use as the starting point for a strip

    for start_point in all_adj:
        if start_point.nextTri == None and start_point.prevTri == None:
            count += 1
            buildTristrip(triangles, start_point)
