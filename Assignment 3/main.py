...
# area function where input parameters are two edges
# values strategically passed into the function to
# ensure duplicate vertex isn't considered
def Area(edge1, edge2):
    return triangleArea(edge1[0].coords, edge1[1].coords, edge2[1].coords)
...
def buildTriangles( slice0, slice1 ):
    ...
    # use nested for loop to find closest pair of vertices
    for i in range(len(slice0.verts)):
        for j in range(len(slice1.verts)):
            temp = length(subtract(slice0.verts[i].coords, slice1.verts[j].coords))
            # check for lowest value or if it's the first value
    ...
    # create new top list
    # index 0 of new list is starting top point
    # append other vertices after that
    # append starting vertex to the end of the list as well
    for i in range(len(slice0.verts) + 1):
        if i == len(slice0.verts):
            tn.append(slice0.verts[topStart])

        elif topStart + i > len(slice0.verts) - 1:
            tn.append(slice0.verts[topStart + i - len(slice0.verts)])

        else:
            tn.append(slice0.verts[topStart + i])

    # link new top list together
    for i in range(len(slice0.verts)):
        tn[i].nextV = tn[i + 1]


    # create new bottom list 
    # index 0 of new list is starting bottom point
    # append other vertices after that
    # append starting vertex to the end of the list as well
    for i in range(len(slice1.verts) + 1):
        if i == len(slice1.verts):
            bn.append(slice1.verts[botStart])

        elif botStart + i > len(slice1.verts) - 1:
            bn.append(slice1.verts[botStart + i - len(slice1.verts)])

        else:
            bn.append(slice1.verts[botStart + i])

    # link new bottom list together
    for i in range(len(slice1.verts)):
        bn[i].nextV = bn[i + 1]
    ...
    minArea = [[None]*len(tn) for i in range(len(bn))]
    minDir  = [['']*len(tn) for i in range(len(bn))]
    ...
    # only need one input parameter
    def minAreaRow0(i):
        if i == 0:
            return 0
        minArea[0][i] = minAreaRow0(i-1) + Area((tn[i-1],bn[0]),(bn[0],tn[i]))
        minDir[0][i] = '-'
        return minArea[0][i]
    ...
    def minAreaCol0(j):
        if j == 0:
            return 0
        minArea[j][0] = minAreaCol0(j-1) + Area((bn[j-1],tn[0]),(tn[0],bn[j]))
        minDir[j][0] = '|'
        return minArea[j][0]
    ...
    def minAreaGeneral(i,j):
        if minArea[j][i] != None: #If minArea has already been calculated before,
            return minArea[j][i]

        elif minArea[j][i-1] != None and minArea[j-1][i] != None:
            #Above and before calculated
            leftEQN = minArea[j][i-1] + Area((tn[i-1],bn[j]),(bn[j],tn[i]))
            rightEQN = minArea[j-1][i] + Area((tn[i],bn[j-1]),(tn[i],bn[j]))

        elif minArea[j][i-1] != None:
            #Column before already calculated
            leftEQN = minArea[j][i-1] + Area((tn[i-1],bn[j]),(bn[j],tn[i]))
            rightEQN = minAreaGeneral(i,j-1) + Area((tn[i],bn[j-1]),(tn[i],bn[j]))
        elif minArea[j-1][i] != None:
            #Row above has already been calculated
            leftEQN = minAreaGeneral(i-1,j) + Area((tn[i-1],bn[j]),(bn[j],tn[i]))
            rightEQN = minArea[j-1][i] + Area((tn[i],bn[j-1]),(tn[i],bn[j]))
        else:
            #Middle of nowhere
            leftEQN = minAreaGeneral(i-1,j) + Area((tn[i-1],bn[j]),(bn[j],tn[i]))
        ...
        minArea[j][i] = min(leftEQN,rightEQN)

        # set min direction
        if leftEQN == minArea[j][i]:
            minDir[j][i] = '-'
        else:
            minDir[j][i] = '|'
    ...
    # call functions on the new lists
    minAreaRow0(len(tn)-1)
    minAreaCol0(len(bn)-1)
    minAreaGeneral(len(tn)-1, len(bn)-1)
    ...
    def printMinAreas():
        ...
        print()
        print("         ",end ='')
        for i in range(len(tn)): #Print header row
            print("\t", i, end ='')
        ...
        for i in range(len(bn)):
            ...
            for j in range(len(tn)): #Print row contents:
                print("\t", round(minArea[i][j]),minDir[i][j], end ='')
            print()

    printMinAreas()
    # comment above line out to remove printing
    ...
    # pass in 2 edges, construct a triangle out of
    # the 3 desired edges, not using the duplicate vertex
    # which we put in edge2[0]
    def constructTriangle(edge1, edge2):
        verts = [edge1[0], edge1[1], edge2[1]]
        return Triangle(verts)

    i0 = len(tn)-1
    j0 = len(bn)-1
    while minDir[j0][i0] != '.':
        if minDir[j0][i0] == '-': #from left
            triangles.append(constructTriangle((tn[i0-1],bn[j0]),(bn[j0],tn[i0])))
            i0 -= 1

        else: #from above
            triangles.append(constructTriangle((tn[i0],bn[j0-1]),(tn[i0],bn[j0])))
            j0 -= 1
